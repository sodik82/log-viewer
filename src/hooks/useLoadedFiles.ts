    ...
    const loaded = await Promise.all(
      arr.map(async (file): Promise<LoadedFile> => {
        ...
        try {
          ...
          return { id: generateId(), name: file.name, entries: [], timestampField: null, error: 'Failed to read file' };
        }

        if (!content.trim()) {
          return { id: generateId(), name: file.name, entries: [], timestampField: null, error: 'File is empty' };
        }
        ...
        try {
          ...
          return { id: generateId(), name: file.name, entries: [], timestampField: null, error: 'Parse error' };
        }
      })
    );
